...
# Source the simple_services.sh file, if it exists in $PROGRAM_DIR.
# The $PROGRAM_DIR/simple_services.sh file will only be present in the
# development environments, so this is a way to simply use
# $PROGRAM_DIR/fireqos.sh, while developing, without having to autoreconf
# in order to generate $PROGRAM_DIR/fireqos.
if [ "`${BASENAME_CMD} "$0"`" = "$PROGRAM_DIR/fireqos.sh" ] && [ -f $PROGRAM_DIR/simple_services.sh ]
then
	. $PROGRAM_DIR/simple_services.sh
fi
